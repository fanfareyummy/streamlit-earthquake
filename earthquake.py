...
# 🚨 [핵심 변경] 모든 텍스트 요소를 순수 검정색(#000000)으로 강제 격파하는 전역 CSS
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght=400;700&display=swap');

    /* 앱 전체 기본 글자색을 완전한 검정색으로 지정 */
    .stApp { 
    ...
    /* 배경과 겹쳐서 안 보이던 핵심 메트릭 숫자를 순수 검정색으로 강제 오버라이딩 */
...
st.markdown("""
    <div class='hospital-header'>
        <div class='hospital-title'>🩺 스마트 의료 통합 관제 센터 <span>[V10.2 REAL_BLACK]</span></div>
        <div class='status-badge'>● 전역 검정색 폰트 강제 고정 프로토콜 가동</div>
    </div>
""", unsafe_allow_html=True)
...
with col_globe:
    ...
    st.markdown("""
        <div style='background: #F8FAFC; border: 1px solid #E2E8F0; padding: 10px 15px; border-radius: 10px; font-size: 0.85rem; color: #000000; font-weight: 500; line-height: 1.5; margin-bottom: 10px;'>
            <b>💡 CDC 감염병 변이 통계 보강:</b> 전 세계 하수 기반 역학 조사 결과, 오미크론 하위 계통의 변이 속도가 지질 외벽 안정성에 미치는 상관관계가 상기 차트와 같이 도출되었습니다.
        </div>
    """, unsafe_allow_html=True)
    
    # 🚨 [검정색 흑화 완료] 이제 텍스트가 순수 검정색으로 뚜렷하게 가독성을 확보합니다.
...
with c_result:
    ...
    if not near_df.empty:
        ...
        st.markdown(f"""
            <div style='background:{res_color}15; color:#000000; border:2px solid {res_color}; padding:10px; border-radius:10px; text-align:center; font-weight:700; margin-bottom:10px;'>
                판독 등급: {res_text}
            </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("<div style='background:#F1F5F9; color:#000000; padding:10px; border-radius:10px; text-align:center; font-size:0.85rem; font-weight:700;'>측정 범위 내 데이터 분석 불가능</div>", unsafe_allow_html=True)
